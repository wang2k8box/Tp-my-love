...
const defaultHeartImages = Array.from({ length: 83 }, (_, i) => `images/img${i + 1}.jpg`);
...
function createTextRings() {
  ...
  for (let i = 0; i < numRings; i++) {
    ...
    const fontSize = Math.max(120, 0.8 * textureHeight);
    ...
    ctx.fillText(fullText, 0, textureHeight * 0.72);
    ...
    ringGroup.userData = {
      ...
      speed: 0.002 + 0.00016, // Tốc độ quay
...
function createHintText() {
  ...
  context.shadowColor = '#f09fcdff';
  context.shadowBlur = 5;
  context.lineWidth = 2;
  context.strokeStyle = 'rgba(255, 193, 216, 0.8)';
  context.strokeText(text, canvasSize / 2, canvasSize / 2);
  context.shadowColor = '#d0a7ecff';
  context.shadowBlur = 5;
  context.lineWidth = 2;
  context.strokeStyle = 'rgba(201, 161, 236, 0.5)';
  context.strokeText(text, canvasSize / 2, canvasSize / 2);
